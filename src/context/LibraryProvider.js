...
export function LibraryProvider({ children }) {
  ...
  useEffect(() => {
    async function getLikedVideos() {
      ...
      try {
        let response = await axios.get(
          `https://trek-backend.herokuapp.com/likedvideo/${currentUser?._id}`
        );
  ...
  useEffect(() => {
    async function getWatchLaterVideos() {
      ...
      try {
        let response = await axios.get(
          `https://trek-backend.herokuapp.com/watchlater/${currentUser?._id}`
        );
  ...
  useEffect(() => {
    async function getPlaylist() {
      ...
      try {
        let response = await axios.get(
          `https://trek-backend.herokuapp.com/playlist/${currentUser?._id}`
        );
        if (response.status === 200) {
          let {
            data: { data }
  ...
  async function handleAddToLikedVideos({ userId, videoId }) {
    ...
    try {
      let response = await axios.post(
        `https://trek-backend.herokuapp.com/likedvideo/${userId}`,
        {
          videoId: videoId
        }
      );
  ...
  async function handleAddToWatchLaterVideos({ userId, videoId }) {
    ...
    try {
      let response = await axios.post(
        `https://trek-backend.herokuapp.com/watchlater/${userId}`,
        {
          videoId: videoId
        }
      );
  ...
  async function handleCreatePlaylist({ userId, playlistname }) {
    ...
    try {
      let response = await axios.post(
        `https://trek-backend.herokuapp.com/playlist/${userId}`,
        {
